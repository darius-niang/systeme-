	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = "Vous avez chronométré pour aujourd'hui";
				}
				else{
					//updates
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = "Vous avez expiré le temps d'aujourd'hui";
					}
					else{
			...
			$output['error'] = true;
			$output['message'] = "ID de l'employé est introuvable";
		}
